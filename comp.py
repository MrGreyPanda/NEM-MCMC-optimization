...
class Comp:
    ...
    def local_ll_sum_b_inv(self, x, weights, i, k, local_vec, c_vec):
        weights[i][k] = x
        weights = utils.order_arr(self.order, self.exp_parent_weights(weights) * self.mask)
        # B = solve_triangular(self.I - weights, self.I, lower=True)
        B = weights
        # B = inv(self.I - np.exp(weights) * self.mask)
        B = B / (1.0 + B)
    ...
    def calculate_local_optimum_b_inv(self, i, k, order_weights, bounds, expit_weights, weights):
        ...
        c_vec = a_vec / b_vec
        res = minimize(self.local_ll_sum_b_inv, x0=weights[i][k], bounds=bounds, options={'eps': 1e-3}, args=(weights, i, k, local_vec, c_vec), method='L-BFGS-B', tol=0.01)
        if res.success is False:
            raise Exception(f"Minimization not successful, Reason: {res.message}")
        return res.x
    ...
    def opt_b(self, weights, bounds):
        ...
        # inv_weights = solve_triangular(self.I - np.exp(new_parent_weights), self.I, lower=True)
        # expit_weights = inv_weights / (1.0 + inv_weights)
        expit_weights = np.exp(new_parent_weights) / (1.0 + np.exp(new_parent_weights))
